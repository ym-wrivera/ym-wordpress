...
class WPMS_settings 
{
	...
	function WPMS_mail_insert_options() {
		...
		<div class="clear"></div>
	<p Style="float:left;">If you like <strong>WP Mail Smtp Mailer</strong> please leave us a <a href="https://wordpress.org/plugins/wp-mail-smtp-mailer/" target="_blank" data-rated="Thanks :)">★★★★★</a> rating. A huge thank you from <a href="http://ciphercoin.com" target="_blank">cipherCoin</a> in advance!</p>
		...
		if( isset($_POST['to_email']) ){
            ...
            if ( wp_verify_nonce( $nonce_vrfy, 'WPMS-mail-send') ){
				...
				if( $sent ){
				    ...
				    if ( ! get_user_meta($user_id, 'wp_mail_smtp_ignore_notice' ) ) {

				        echo '<div class="updated"><p>'; 

				        printf(__('Awesome, you\'ve been using <b>Mail SMTP Mailer</b>. May we ask you to give it a 5-star rating on WordPress? | <a href="%2$s" target="_blank">Ok, you deserved it</a> | <a href="%1$s">I alredy did</a> | <a href="%1$s">No, not good enough</a>'), 'options-general.php?page=wp-mail-smtp-mailer&wp_smtp_mailer_nag_ignore=0','https://wordpress.org/plugins/wp-mail-smtp-mailer/');
				        echo "</p></div>";
				    }
...
function wp_mail_admin_notice() {
	...
	$install_date = get_option( 'wp_mailer_install_date', '');
...
function wp_smtp_mailer_nag_ignore() {
    ...
    if ( isset($_GET['wp_smtp_mailer_nag_ignore']) && '0' == $_GET['wp_smtp_mailer_nag_ignore'] ) {

        add_user_meta($user_id, 'wp_mail_smtp_ignore_notice', 'true', true);
    }
}
