...
class Admin_Handler {
	...
	public function maybe_rollback_plugin() {
		...
		require_once dirname( $this->bot->file_path() ) . '/includes/uninstall.php';
	...
	/**
	 * Maybe automatically update the plugin from wp.org.
	 * Can be disabled by setting the MERGEBOT_DISABLE_AUTO_UPDATES constant to true.
	 *
	 * @param bool   $update
	 * @param object $item
	 *
	...
	public function maybe_auto_update_plugin( $update, $item ) {
		if ( $this->bot->slug() === $item->slug && false === Config::disable_auto_updates() ) {
			return true;
		}
