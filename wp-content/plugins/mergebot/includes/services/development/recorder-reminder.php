...
class Recorder_Reminder {
	...
	protected function can_display_popup() {
		...
		$dismissed = get_option( $this->dismissed_key, false );
	...
	public function set_background_recorded_query( $query ) {
		...
		if ( false !== get_option( $this->recorded_query_key, false ) ) {
			// Abort if we have already set a previous query ID
			return false;
		}

		$result = update_option( $this->recorded_query_key, $query->to_array() );
	...
	public function ajax_dismiss_popup() {
		...
		// Clear Query
		delete_option( $this->recorded_query_key );
		// Set the dismissed flag for the popup
		update_option( $this->dismissed_key, time() + ( $minutes * 60 ) );
	...
	public function send_background_recorded_query_to_app( $result, $recording_id ) {
		...
		$query = get_option( $this->recorded_query_key );
		if ( false === $query ) {
