...
class Recorder_Presenter {
	...
	protected function should_display_recorder_button() {
		...
		if ( $this->recorder_handler->query_limit_reached() ) {
			// Beta query limit reached, stop recording and hide button
			return false;
		}
		...
		if ( ! is_admin_bar_showing() || ! current_user_can( $this->bot->capability() ) ) {
