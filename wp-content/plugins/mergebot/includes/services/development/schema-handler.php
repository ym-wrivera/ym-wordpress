...
class Schema_Handler {
	...
	protected function get_schema_data( $key, $bust_cache = false ) {
		...
		if ( false !== ( $cached_data = get_transient( $this->bot->slug() . '_' . $key ) ) && false === $bust_cache ) {
			return $cached_data;
		}
		...
		if ( ! is_wp_error( $app_data ) ) {
			...
			set_transient( $this->bot->slug() . '_' . $key, $app_data, 12 * HOUR_IN_SECONDS );
			update_option( $this->bot->slug() . '_' . $key, $app_data );

			return $app_data;
		}

		// Can't get a refresh of data from the app, use backup option
		if ( false !== ( $backup_data = get_option( $this->bot->slug() . '_' . $key, false ) ) ) {
