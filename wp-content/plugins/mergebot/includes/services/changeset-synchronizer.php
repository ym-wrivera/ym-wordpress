...
class Changeset_Synchronizer {
	...
	public function refresh_changeset( Changeset $old_changeset, App_Changeset $new_changeset ) {
		...
		// Update query limit changeset ID
		$this->maybe_update_query_limit( $new_changeset->id );
	...
	public function remove( $changeset ) {
		...
		$query_limit = get_option( $this->bot->slug() . '_query_limit' );
		if ( false !== $query_limit && $changeset_id == $query_limit ) {
			// Remove query limit flag if we are closing that changeset
			delete_option( $this->bot->slug() . '_query_limit' );
			Notice::delete_by_id( 'beta-query-limit' );
		}
	}
	 ...
	 * @param int $id
	 */
	protected function maybe_update_query_limit( $id ) {
		if ( ! $this->bot->is_beta() ) {
			return;
		}
		...
		if ( false !== $query_limit && 0 === (int) $query_limit ) {
			...
			update_option( $this->bot->slug() . '_query_limit', $id );
		}
	}
