...
class Send_Queries extends Abstract_Job {
	...
	protected function clean_up_unprocessed() {
		...
		foreach ( $unprocessed_queries as $query ) {
			if ( (int) get_transient( $this->identifier . '_error_count' ) >= $this->error_retry_limit ) {
			...
			if ( $query->is_insert() && ! $this->schema_handler->is_table_defined_in_schema( $query->insert_table ) ) {
				// Table out of scope of beta, delete query and associated data
				$query->clear();
	...
	protected function send_queries_to_app( $data ) {
		...
		if ( ! is_wp_error( $result ) ) {
			...
			$error_title     = __( 'App Communication Error' );
			$error_msg       = __( 'Could not send queries to the app.' );
			$error_msg_extra = ' ' . $result->get_error_message();
			$error_data      = $data;
		}
	...
	protected function handle_query_limit_reached( $result ) {
		...
		update_option( $this->bot->slug() . '_query_limit', $changeset_id );
	}
}